...
- (void)prepare
{
    CAEmitterLayer *snowLayer = [self makeEmitterAtPoint:CGPointMake(20, -50)];
    CAEmitterCell *snowCell1 = [self makeEmitterCellWithParticle:@"bg_snow"];
...
- (CAEmitterLayer *)makeEmitterAtPoint:(CGPoint)point {
	...
	emitterLayer.emitterPosition = CGPointMake(CGRectGetMidX([self frame]), -50);
	emitterLayer.emitterZPosition = 0;
	emitterLayer.emitterSize = CGSizeMake([self bounds].size.width, 5.00);
...
- (CAEmitterCell *)makeEmitterCellWithParticle:(NSString *)name
{
	...
	emitterCell.scale = 0.52;
	emitterCell.scaleRange = 0.2;
	emitterCell.scaleSpeed = -0.005;
    
	emitterCell.color = [[UIColor whiteColor] CGColor];
    //[[UIColor colorWithRed:0.77 green:0.55 blue:0.60 alpha:0.55] CGColor];
	...
	emitterCell.lifetimeRange = 4.37;
	emitterCell.birthRate = 0;
	emitterCell.velocity = -10.00;
